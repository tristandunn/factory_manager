...
module FactoryManager
  ...
  class Generator
    ...
    def initialize(strategy:)
      @alias_name = FactoryManager.configuration.alias_name
      @associations = {}
      @locals = {}
      @results = {}
      @strategy = strategy
    ...
    def _add_association(record, name:)
      @associations[name] ||= []
      @associations[name] << record

      yield

      @associations[name].pop
    end
    ...
    def _generate_factory(name, *arguments)
      arguments.push(
        _associations_for(name).merge(
          arguments.extract_options!.except(@alias_name)
        )
      )
    ...
    def method_missing(method, *arguments, &block)
      ...
      if _assignment_method?(method)
        ...
        _add_association(record, name: options.fetch(@alias_name, method)) do
